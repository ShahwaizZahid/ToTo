  ...
  Future<void> fetchOrders() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final userOrders = data.where((order) => order['userId'] == userId).toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title:  Text("Orders",style: TextStyle(color: Theme.of(context).colorScheme.inversePrimary),),
        centerTitle: true,
      ...
      body: isLoading
        ...
        decoration: BoxDecoration(
          color: Theme.of(context).colorScheme.background
            ...
            ...pendingOrders.map((order) => _buildOrderSection(order, context)),
            const SizedBox(height: 24),
            _buildSectionTitle(context, "Success Orders"),
            if (successOrders.isEmpty)
              const Text("No successful orders"),
            ...successOrders.map((order) => _buildOrderSection(order, context)),
                    ],
                  ),
          ),
    );
  ...
  Widget _buildOrderSection(Map<String, dynamic> order, BuildContext context) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        side: BorderSide(
          color: Theme.of(context).colorScheme.inversePrimary, // Border color here
